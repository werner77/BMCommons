...
- (void)testEmailValidation{
	...
	XCTAssertFalse(valid, @"Expected validation to fail for email without @ sign");

	valid = ([@"edwin@mirabeau(dot)nl" stringByMatching:regExpr] != nil);
	XCTAssertFalse(valid, @"Expected validation to fail for email without . sign");

	valid = ([@"edwin@.mirabeau.nl" stringByMatching:regExpr] != nil);
	XCTAssertFalse(valid, @"Expected validation to fail for email without characters between @ and .");

	valid = ([@"edwin@mirabeau.nl" stringByMatching:regExpr] != nil);
	XCTAssertTrue(valid, @"Expected validation to succeed for normal email");

	valid = ([@"edwin.vermeer@mirabeau.nl" stringByMatching:regExpr] != nil);
	XCTAssertTrue(valid, @"Expected validation to succeed for normal email with a . before the @");

	valid = ([@"edwin.vermeer@iphone.mirabeau.nl" stringByMatching:regExpr] != nil);
	XCTAssertTrue(valid, @"Expected validation to succeed for normal email with multiple .");
