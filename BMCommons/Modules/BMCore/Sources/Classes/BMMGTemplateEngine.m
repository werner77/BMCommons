...
@interface BMMGTemplateEngine (PrivateMethods)
...
{
	return [[BMMGTemplateEngine alloc] init];
}
...
- (id)init
{
	if ((self = [super init])) {
		...
		[self loadMarker:[[BMMGTemplateStandardMarkers alloc] initWithTemplateEngine:self]];
		[self loadFilter:[[BMMGTemplateStandardFilters alloc] init]];
	}
...
but current block was started by \"%@\" marker", 
			...
			if (val && _outputDisabledCount == 0) {
				...
				if (filter) {
					NSObject <BMMGTemplateFilter> *filterHandler = [_filters objectForKey:filter];
					if (filterHandler) {
			...
			if (openBlocks > 0) {
				NSString *errMsg = [NSString stringWithFormat:@"Finished processing template, but %tu %@ left open (%@).", 
									openBlocks, 
