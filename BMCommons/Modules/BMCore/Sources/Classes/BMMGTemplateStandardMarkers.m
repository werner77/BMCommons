        ...
        if (args && [args count] >= 3) {
            ...
            if (isRange) {
                ...
                if (obj && [obj respondsToSelector:@selector(objectEnumerator)] && [obj respondsToSelector:@selector(count)]) {
                    endIndex = BMShortenUIntToIntSafely([(NSArray *)obj count], nil);
                    if (endIndex > 0) {
                        loopEnumObject = obj;
                        valid = YES;
                    }
            ...
            if (valid) {
                *blockStarted = YES;
                ...
                NSMutableDictionary *loopVars = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                 ...
                                                 [NSNumber numberWithInteger:currentIndex+1], FOR_LOOP_CURR_1_INDEX,
        ...
        if (args && ([args count] == 1 || [args count] == 3)) {
            *blockStarted = YES;
            ...
            if ([args count] == 1) {
                ...
                if (!firstNumeric) {
                    num1 = (NSInteger)[engine resolveVariable:firstArg];
                }
                if (!secondNumeric) {
                    num2 = (NSInteger)[engine resolveVariable:secondArg];
                }
            ...
            /*
            if (!argTrue && args.count == 1 && [[args objectAtIndex:0] isEqual:@"mapping.isEnumeration"]) {
                NSLog(@"If Args: %@", args);
            }
            */
        ...
        if (args && [args count] > 0) {
            for (NSString *className in args) {
                ...
                if (class && [(id)class isKindOfClass:[NSObject class]]) {
                    if ([class conformsToProtocol:@protocol(BMMGTemplateFilter)]) {
                        // Instantiate and load filter.
                        NSObject <BMMGTemplateFilter> *obj = [[class alloc] init];
                        [engine loadFilter:obj];
                    } else if ([class conformsToProtocol:@protocol(BMMGTemplateMarker)]) {
                        // Instantiate and load marker.
                        NSObject <BMMGTemplateMarker> *obj = [[class alloc] initWithTemplateEngine:engine];
