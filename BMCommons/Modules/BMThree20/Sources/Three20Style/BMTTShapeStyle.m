...
+ (BMTTShapeStyle*)styleWithShape:(BMTTShape*)shape next:(BMTTStyle*)next {
  BMTTShapeStyle* style = [[[self alloc] initWithNext:next] autorelease];
  style.shape = shape;
  return style;
}
...
- (void)draw:(BMTTStyleContext*)context {
  ...
  context.contentFrame = BMTTRectInset(context.contentFrame, shapeInsets);
