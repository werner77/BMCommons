...
+ (NSDictionary *)fieldMappings {
	...
	@synchronized ([BMAbstractMappableObject class]) {
		...
		if (!dict) {
			...
			dict = [BMFieldMapping parseFieldDescriptorDictionary:[self fieldMappingDictionary] withNamespaces:mappingNamespaces];
...
+ (NSString *)rootElementName {
    NSException *ex = [NSException exceptionWithName:@"RootElementNotDefinedException" reason:@"Subclasses should override rootElementName for this method to succeed" userInfo:nil];
    @throw ex;
}
...
@implementation BMAbstractMappableObject(Protected)
