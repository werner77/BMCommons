 ...
 * Additions.
 */
BMTT_FIX_CATEGORY_BUG(UINavigationControllerAdditions)

@implementation UINavigationController (BMTTCategory)
...
- (void)addSubcontroller:(UIViewController*)controller animated:(BOOL)animated
  ...
  if (animated && transition) {
    if ([self isKindOfClass:[BMNavigationController class]]) {
      [(BMNavigationController*)self pushViewController: controller
...
- (NSString*)keyForSubcontroller:(UIViewController*)controller {
  NSUInteger controllerIndex = [self.viewControllers indexOfObject:controller];
  if (controllerIndex != NSNotFound) {
    return [NSNumber numberWithUnsignedInteger:controllerIndex].stringValue;
