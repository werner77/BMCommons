...
// UI
#import "Three20UI/BMTTScrollViewDelegate.h"
#import "Three20UI/BMTTScrollViewDataSource.h"
...
- (void)dealloc {
  ...
  BMTT_RELEASE_SAFELY(_animationStartTime);
  BMTT_RELEASE_SAFELY(_pages);
  BMTT_RELEASE_SAFELY(_pageQueue);

  BMTT_RELEASE_SAFELY(_touch1);
  BMTT_RELEASE_SAFELY(_touch2);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (int)flickDirection:(BOOL)ignoreFirstLastPageBoundaries {
    BOOL isFirstPage = !ignoreFirstLastPageBoundaries && [self isFirstPage];
    BOOL isLastPage = !ignoreFirstLastPageBoundaries && [self isLastPage];
  if (!self.flipped) {
    if (_pageEdges.left > kFlickThreshold && !isFirstPage) {
      return -1;

    } else if (_pageEdges.right < -kFlickThreshold && !isLastPage) {
      return 1;
    ...
    if (_pageEdges.left > kFlickThreshold && !isLastPage) {
      return 1;

    } else if (_pageEdges.right < -kFlickThreshold && !isFirstPage) {
      return -1;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BOOL)flicked {
    return ([self flickDirection:NO] != 0);
}
...
- (void)layoutPage {
  ...
  if (nil != page) {
    CGAffineTransform rotation = BMTTRotateTransformForOrientation(_orientation);
...
- (void)startAnimationTo:(UIEdgeInsets)edges duration:(NSTimeInterval)duration {
  if (!_animationTimer) {
    ...
    BMTT_INVALIDATE_TIMER(_tapTimer);
...
- (void)touchesMoved:(NSSet*)touches withEvent:(UIEvent *)event {
  ...
  BMTT_INVALIDATE_TIMER(_holdingTimer);
  ...
  if (_scrollEnabled && !_holding && _touchCount && !_animationTimer) {
    ...
    if (![self edgesAreZoomed:pageEdges] || self.canZoom) {
      ...
      int flickDirection = [self flickDirection:YES];
      if (([self isLastPage] && flickDirection == 1) ||
          ([self isFirstPage] && flickDirection == -1)) {
          if ([self.delegate respondsToSelector:@selector(scrollView:flickedBoundary:)]) {
            _ignoreBoundaries = [self.delegate scrollView:self flickedBoundary:[self isLastPage]];
          }
      }
  }
}
...
//  UIInterfaceOrientation orientation = BMTTDeviceOrientation();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setDataSource:(id<BMTTScrollViewDataSource>)dataSource {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)scrollToCenterPage:(BOOL)animated {
    UIEdgeInsets edges = [self pageEdgesForAnimation];
    NSTimeInterval dur = self.flicked ? kFlickDuration : kBounceDuration;
    if (!animated) {
        dur = 0.0;
