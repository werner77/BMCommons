...
#import "Three20UI/BMTTModelViewController.h"

// UICommon
#import "Three20UICommon/UIViewControllerAdditions.h"

// Network
#import "Three20Network/BMTTModel.h"
...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@implementation BMTTModelViewController
...
- (void)dealloc {
  ...
  BMTT_RELEASE_SAFELY(_model);
  BMTT_RELEASE_SAFELY(_modelError);
  [super dealloc];
}
...
- (void)createInterstitialModel {
  self.model = [[[BMTTModel alloc] init] autorelease];
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)modelDidChange:(id<BMTTModel>)model {
  if (model == _model) {
    [self refresh];
  }
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)modelDidBeginUpdates:(id<BMTTModel>)model {
  if (model == _model) {
    [self beginUpdates];
  }
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)modelDidEndUpdates:(id<BMTTModel>)model {
  if (model == _model) {
    [self endUpdates];
  }
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setModel:(id<BMTTModel>)model {
  if (_model != model) {
    [_model.delegates removeObject:self];
    [_model release];
    _model = [model retain];
...
- (void)reload {
  _flags.isViewInvalid = YES;
  [self.model load:BMTTURLRequestCachePolicyNetwork more:NO];
}
...
- (void)refresh {
  ...
  if (!loading && !loaded && [self shouldLoad]) {
    [self.model load:BMTTURLRequestCachePolicyDefault more:NO];

  } else if (!loading && loaded && [self shouldReload]) {
    [self.model load:BMTTURLRequestCachePolicyNetwork more:NO];

  } else if (!loading && [self shouldLoadMore]) {
    [self.model load:BMTTURLRequestCachePolicyDefault more:YES];
