...
@implementation NSData (BMCompression)
...
- (NSData *)bmZlibInflate
{
	...
	strm.avail_in = BMShortenUIntSafely([self length], nil);
	strm.total_out = 0;
	strm.zalloc = Z_NULL;
	strm.zfree = Z_NULL;
	...
	while (!done)
	{
		...
		strm.avail_out = BMShortenUIntSafely([decompressed length] - strm.total_out, nil);
...
- (NSData *)bmZlibDeflate
{
	...
	do {
		...
		strm.avail_out = BMShortenUIntSafely([compressed length] - strm.total_out, nil);
...
- (uint32_t)bmCRC32
{
	...
	bytes = [self bytes];
	max = BMShortenUIntSafely([self length], nil);
	crcval = 0xffffffff;
	for (x = 0, y = max; x < y; x++) {
