...
- (void)sendTestRequestWithCompletion:(BMHTTPRequestCompletionBlock)completion {
    ...
    [_httpRequest send];
    [self prepare:@selector(testConnectionError)];
...
- (void)sendTestRequestWithIndex:(NSUInteger)index maxCount:(NSUInteger)maxCount {
    if (index < maxCount) {
        ...
        [self notify:kGHUnitWaitStatusSuccess forSelector:@selector(testConnectionError)];
    }
}
...
//[self notify:kGHUnitWaitStatusFailure forSelector:@selector(testAsyncHttpRequest)];

// [[NSString alloc] initWithData:theRequest.replyData encoding:NSUTF8StringEncoding]);
//
